// ============================================================
// 오늘학교 v1.6.4 Hakdol Core v0.2.0 적용
// Hakdol Core의 NEIS 공통 캐시(NeisCache 호환 별칭) + 오늘 정보 우선 표시 + 프록시 워밍업
// ============================================================
...
const els = {
  ...
  helpDoneBtn: document.querySelector("#helpDoneBtn"),
...
function warmApiProxy() {
  try {
    const controller = new AbortController();
    const timer = window.setTimeout(() => controller.abort(), 8000);
    fetch(`${API_CONFIG.baseUrl}/`, { cache: "no-store", signal: controller.signal })
      .catch(() => {})
      .finally(() => window.clearTimeout(timer));
  } catch (error) {
    // 프록시 워밍업 실패는 실제 조회 흐름에 영향을 주지 않습니다.
  }
}
...
function closeHelpDialog() {
  if (!els.helpDialog) return;
  if (typeof els.helpDialog.close === "function" && els.helpDialog.open) {
...
function bindEvents() {
  ...
  if (els.tomorrowDetailBtn) {
    els.tomorrowDetailBtn.addEventListener("click", async () => {
      ...
      state.currentDate = new Date(tomorrow.getFullYear(), tomorrow.getMonth(), 1);
      state.selectedDate = formatDateKey(tomorrow);
      state.activeTab = "calendar";
      renderView();
      await loadMonthData();
      renderAll();
      requestAnimationFrame(() => scrollToViewSection(document.querySelector("#detailArea"), true));
    });
  }
...
async function fetchSchools(keyword, officeCode, signal) {
  ...
  if (cached.hit && Array.isArray(cached.data)) return cached.data;
  ...
  try {
    ...
    NeisCache.set(cacheKey, schools, { expiresAt: Date.now() + SCHOOL_SEARCH_CACHE_MS });
    return schools;
  } catch (error) {
    const stale = NeisCache.get(cacheKey, { allowStale: true });
    if (stale.hit && Array.isArray(stale.data)) return stale.data;
    throw error;
  }
}
...
async function fetchSchedulesForDateKey(dateKey, { forceRefresh = false } = {}) {
  ...
  const monthSchedules = Array.isArray(result.data) ? result.data : [];
  return {
    data: monthSchedules.filter((item) => item.date === dateKey),
    stale: Boolean(result.stale)
  };
}

async function fetchMealForDateKey(dateKey, { forceRefresh = false } = {}) {
  if (!state.selectedSchool || !dateKey) return { data: null, stale: false };
  const context = getNeisContext();
  const cacheKey = NeisCache.keys.meal(context.schoolCode, dateKey);
  const result = await NeisCache.getOrFetch(cacheKey, async () => {
    const params = new URLSearchParams({
  ...
  return { data: result.data ?? null, stale: Boolean(result.stale) };
}

async function loadTodayMealFast({ forceRefresh = false } = {}) {
  if (!state.selectedSchool) return;

  const todayKey = formatDateKey(new Date());
  const schoolCode = state.selectedSchool.schoolCode;
  state.todayMealStatus = "loading";
  renderTodaySummary();

  try {
    const result = await fetchMealForDateKey(todayKey, { forceRefresh });
    if (state.selectedSchool?.schoolCode !== schoolCode) return;
    state.todayMeal = result.data;
    state.todayMealStatus = result.stale ? "stale" : "success";
  } catch (error) {
    if (state.selectedSchool?.schoolCode !== schoolCode) return;
    state.todayMeal = null;
    state.todayMealStatus = "error";
  } finally {
    if (state.selectedSchool?.schoolCode === schoolCode) renderTodaySummary();
  }
}
...
async function loadTomorrowPreview() {
  const sequence = ++tomorrowPreviewSequence;
  if (!state.selectedSchool) {
    state.tomorrowSchedules = [];
    state.tomorrowMeal = null;
    state.tomorrowTimetable = [];
  ...
  const schoolCode = state.selectedSchool.schoolCode;
  const tomorrowKey = formatDateKey(getTomorrowDate());
  const isCurrent = () => sequence === tomorrowPreviewSequence
...
async function loadMonthData() {
  ...
  const prioritizeToday = state.activeTab === "today"
    && monthPrefix === todayMonthPrefix
  ...
  if (prioritizeToday) state.todayMealStatus = "loading";
  ...
  // 오늘 화면은 사용자가 가장 먼저 보는 정보부터 요청합니다.
  // 시간표와 오늘 급식을 먼저 출발시키고, 오늘 학사일정에 필요한 월 일정도 동시에 조회합니다.
  let timetableTask = Promise.resolve();
  if (state.selectedDate) {
    state.classSwitcherOpen = false;
    restoreTimetableFromCache();
    timetableTask = loadTimetable();
  } else {
    state.timetable = [];
    state.timetableStatus = "idle";
    state.timetableMessage = "확인할 날짜를 선택해 주세요.";
  ...
  const todayMealTask = prioritizeToday ? loadTodayMealFast() : Promise.resolve();
  ...
  const scheduleTask = (async () => {
    try {
      ...
      if (isCurrentLoad()) {
        updateTodaySnapshot({ syncMeal: false });
        renderCalendar();
        renderScheduleDetail();
        renderTodaySummary();
      }
  ...
  const runMealTask = async () => {
    try {
      ...
      if (isCurrentLoad()) {
        state.meal = state.selectedDate ? (state.mealsByDate[state.selectedDate] || null) : null;
        updateTodaySnapshot({ syncMeal: true });
        renderCalendar();
        renderMealDetail();
        renderTodaySummary();
      }
  ...
  if (prioritizeToday) {
    updateDataLoading(loadingToken, "오늘 정보를 먼저 확인하고 있어요", "오늘 급식 · 학사일정 · 시간표를 우선 불러옵니다.");
    await Promise.allSettled([todayMealTask, scheduleTask, timetableTask]);
    if (!isCurrentLoad()) {
  ...
  if (monthPrefix === todayMonthPrefix) void loadTomorrowPreview();
}
...
function updateTodaySnapshot({ syncMeal = true } = {}) {
  const todayKey = formatDateKey(new Date());
  const todayMonthPrefix = todayKey.slice(0, 7);
  const currentMonthPrefix = `${state.currentDate.getFullYear()}-${pad(state.currentDate.getMonth() + 1)}`;
  ...
  if (syncMeal) {
    const hasFreshDailyMeal = state.todayMealStatus === "success";
    const shouldUseMonthMeal = state.mealStatus === "success" || !hasFreshDailyMeal;
    if (shouldUseMonthMeal) {
      state.todayMeal = state.mealsByDate[todayKey] || null;
      if (["success", "stale", "error"].includes(state.mealStatus)) {
        state.todayMealStatus = state.mealStatus;
      }
    }
  }
}
...
function renderTodaySummary() {
  ...
  if (state.todayMealStatus === "loading") {
    els.todayMealSummary.innerHTML = `<p class="empty">${renderLoadingText("급식정보를 불러오는 중입니다")}</p>`;
  } else if (state.todayMealStatus === "error") {
    els.todayMealSummary.innerHTML = `<p class="empty">오늘 급식정보를 불러오지 못했어요. 잠시 후 다시 시도해 주세요.</p>`;
  } else if (todayMeal && todayMeal.dishes?.length) {
    els.todayMealSummary.innerHTML = `<ul>${todayMeal.dishes.map((dish) => `<li>${escapeHtml(dish)}</li>`).join("")}</ul>${state.todayMealStatus === "stale" ? `<p class="today-more">저장된 조회 결과를 보여드려요.</p>` : ""}`;
  } else {
...
function renderTomorrowPreview() {
  ...
  if (!state.selectedSchool) {
    if (els.tomorrowMealTitle) {
      els.tomorrowMealTitle.innerHTML = buildSummaryTitle("내일 급식", "🍱");
    }
    if (els.tomorrowMealSummary) {
      els.tomorrowMealSummary.innerHTML = `<p class="empty">학교를 선택하면 내일 급식을 미리 볼 수 있어요.</p>`;
    }
  ...
  const tomorrowMeal = state.tomorrowMeal;
  if (els.tomorrowMealTitle) {
    const badge = tomorrowMeal?.calorie
      ? ` <span class="title-badge meal-kcal">${escapeHtml(tomorrowMeal.calorie)}</span>`
      : "";
    els.tomorrowMealTitle.innerHTML = buildSummaryTitle("내일 급식", "🍱", badge);
  }

  if (els.tomorrowMealSummary) {
    if (state.tomorrowMealStatus === "loading") {
      els.tomorrowMealSummary.innerHTML = `<p class="empty">${renderLoadingText("내일 급식을 확인하는 중입니다")}</p>`;
    } else if (state.tomorrowMealStatus === "error") {
      els.tomorrowMealSummary.innerHTML = `<p class="empty">내일 급식정보를 불러오지 못했어요.</p>`;
    } else if (tomorrowMeal?.dishes?.length) {
      els.tomorrowMealSummary.innerHTML = `<ul>${tomorrowMeal.dishes.map((dish) => `<li>${escapeHtml(dish)}</li>`).join("")}</ul>${state.tomorrowMealStatus === "stale" ? `<p class="tomorrow-preview-note">저장된 조회 결과를 보여드려요.</p>` : ""}`;
    } else {
      els.tomorrowMealSummary.innerHTML = `<p class="empty">내일 급식정보가 없어요.</p>`;
    }
  }

  const schedules = state.tomorrowSchedules || [];
  if (els.tomorrowScheduleSummary) {
    if (state.tomorrowScheduleStatus === "loading") {
      els.tomorrowScheduleSummary.innerHTML = `<p class="empty">${renderLoadingText("내일 학사일정을 확인하는 중입니다")}</p>`;
    } else if (state.tomorrowScheduleStatus === "error") {
      els.tomorrowScheduleSummary.innerHTML = `<p class="empty">내일 학사일정을 불러오지 못했어요.</p>`;
